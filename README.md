...
**말로 시키는 초벌컷.** 촬영본 폴더를 [클로드 코드](https://claude.com/claude-code)에 건네면 — 대사를 받아쓰고, 말 없는 구간은 화면을 직접 확인하고, NG·무음·중복 테이크를 판단하고, 헷갈리는 건 물어본 뒤, 프리미어 프로 또는 파이널컷 프로에 편집 타임라인을 띄워 줍니다. 렌더링 없음, 원본 무손상, 영상 업로드 없음.
...
- ✂️ **내용 기준 편집** — 대사(전사본)를 읽고, 말 없는 구간은 화면 프레임을 컨택트 시트로 훑어보며 남길 부분과 지울 부분을 판단합니다. "NG는 마지막 테이크만", "3분으로 압축" 같은 지시를 이해합니다
...
- 👀 **무음 구간은 눈으로 확인** — 말이 없다고 자르지 않습니다. 3초 이상 무음 구간은 프레임을 격자 이미지(컨택트 시트) 한 장으로 모아 직접 보고(B롤·제품 샷·풍경·시연인지) 판단한 뒤에만 분류합니다
...
- 🔐 **프라이버시** — 영상 파일 자체는 어디에도 업로드되지 않습니다. 클로드가 읽는 것은 대사 텍스트와, 무음 구간 확인용 화면 캡처 몇 장뿐입니다
...
/firstcut
   ...
   ├─ ④ 무음 화면 확인 ── 3초+ 무음 구간을 컨택트 시트로 직접 보기
...
**사람 이름·브랜드명·전문용어를 자꾸 잘못 받아써요.**
3중으로 잡습니다. ① 시작할 때 자주 나오는 용어를 물어보고(대본이 있으면 자동 추출) 음성 인식에 반영, ② 전사 후 같은 이름이 다르게 적힌 것("새훈/새 훈")을 클로드가 찾아 정확한 표기를 물어본 뒤, ③ 확인된 교정을 전사본·자막 전체에 일괄 적용하고 몇 건 고쳤는지 보고합니다. 교정 기록은 저장돼 다음에도 재사용됩니다.

**영어 영상도 되나요?**
네. 언어는 자동 감지됩니다.

**B롤이나 제품 샷처럼 말 없는 장면은 잘리지 않나요?**
잘리지 않습니다. 3초 이상의 무음 구간은 자동으로 프레임을 추출해 클로드가 직접 화면을 확인하고, 의미 있는 비주얼이면 살리거나 보류 후보로 올립니다. 내용 구성 보고에도 "무음 비주얼 컷"으로 표시돼 조용히 사라지는 일이 없습니다.
